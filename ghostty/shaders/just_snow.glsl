...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 acc = vec3(0.0);
	float dof = 2.0 * sin(iTime * 0.1);
	for (int i = 0; i < LAYERS; i++) {
		float fi = float(i);
		vec2 q =-uv*(1.0 + fi * DEPTH);
		q += vec2(q.y * (WIDTH * mod(fi * 7.238917, 1.0) - WIDTH * 0.5), SPEED * iTime / (1.0 + fi * DEPTH * 0.10)); //was -SPEED * iTime -> somehow caused the snow to go upwards
